...
`z_filter` フォルダをプロジェクトにコピーするだけで使える：
...
汎用的なフィルタクラス`Z_Filter`に加え、よく使うフィルタクラス(LPF、微分フィルタなど)は、時定数やカットオフ周波数を設定するだけで使えるように用意しているので、特殊な用途以外はテンプレートを使うことがおすすめ：
...
デフォルトでは、フィルタライブラリは何もprint出力しない。内部の計算詳細を確認したい場合は、以下のようにログ出力を有効にできる：
...
フィルタの内部状態をリセットして、特定の出力値から開始することができる。
起動直後、過渡状態からフィルタリングを始めた場合、センサ異常からの復帰時などにフィルタリセットをかける運用を想定。
　※初期状態0からフィルタ計算開始すると、時定数分のフィルタ内部状態の収束時間がかかるため、すぐにフィルタ収束させたい場合にリセット機能を使うこと
...
微分フィルタの場合、定常状態の入力値を `u_ss` パラメータで指定する必要がある。
...
連続時間系の伝達関数の分母・分子係数から離散フィルタを作成できる。係数を与えるだけで、内部で双一次変換により離散化される。
テンプレートに無い伝達関数/フィルタ(ノッチフィルタとか)を作りたい場合は、この機能を使うこと。
若干、伝達関数とかを知ってる上級者向けの機能なので、基本はテンプレート使うのがおすすめ。
　※ノッチフィルタとかの使用頻度が多ければ、こっちもテンプレ化して運用することを想定
...
双一次変換（タスティン変換）では、高周波側で周波数特性が歪む。プリワーピング処理を使用すると、特定の周波数で連続時間系と離散系の周波数特性を一致させることができる。
...
プリワーピングは、カットオフ周波数付近の特性を正確に再現したい場合に有用である。
...
**フィルタ時定数の設定ガイドライン:**
- 最小値: `サンプリング周期 × 2` 以上
- 推奨値: 0.02～0.1秒程度（100Hzサンプリングの場合）
...
2. **重力加速度の扱い**: 加速度センサの出力には重力加速度が含まれていることに注意する。必要に応じて重力成分を除去してから座標変換を行うこと（仮想的に加速度センサ搭載個所を変更したときの加速度センサ値を得たい場合は、センサ値をそのまま変換してよい）
3. **角加速度の精度**: 角速度を数値微分して角加速度を計算するため、ノイズの影響を受けやすくなるので、適切なフィルタ時定数の設定が重要となる（もしノイズがひどい場合は、ADFはサンプリング周期の倍の時定数で固定し、LPF時定数のみを弄るのがおすすめ）
4. **サンプリング周波数**: 高速な運動を扱う場合は、サンプリング周波数を十分に高く設定すること（サンプリング定理に従う）
